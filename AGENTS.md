...
- 根目录放聚合 `pom.xml`；`fund-common` 仅共享标识、错误和审计关联信息；`fund-knowledge` 管文档、Embedding、RAG 与引用；`fund-guardian` 管指标、诊断 Agent、审批与模拟治理；`fund-console` 是正式统一管理工作台；`fund-integration` 的资方接入助手按 ADR-011 暂缓扩展；`fund-experiments` 只保留本地单点实验和历史模拟，不继续承载正式控制台。模块源码遵循 `src/main/java`、`src/test/java`、`src/main/resources`。
...
- 术语：合成指标触发智能守护整条链路统一称“智能守护模拟”。重放语义只保留给失败消息重投（`MetricEventConsumer.replay`）和 Agent 从断点恢复。
...
- 本地 Compose 的 DeepSeek 密钥统一放在仓库外的 `$HOME/.config/fund-gateway-ai-center/deepseek.env`（文件权限 600），由 `docker-compose.yml` 的 `env_file` 注入；禁止把密钥写入仓库、`.env`、代码或日志。该文件缺失时应让 Compose 明确失败，不要回退为空值。
